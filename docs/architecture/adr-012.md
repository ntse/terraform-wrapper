...
4. **Installed binaries are cached on disk** under `~/.terraform-wrapper/versions/<version>/terraform`, so repeat invocations that need the same installed version reuse the binary without re-downloading.
